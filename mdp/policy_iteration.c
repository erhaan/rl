...
;;
;; Summary
;;   Our implementation of policy_iteration procedure for Lab:11
;;   + instructors implemented background code
;;
;; Provides
;;   (policy_iteration (const mdp* p_mdp, double epsilon, double gamma,
                        unsigned int *policy))
*/
#include <stdlib.h>
#include <stdio.h>
                        ...
                        unsigned int *policy)
{
  ...
  double * utilities = (double *) malloc(sizeof(double) * num_states);
  
  //Initilizing each state to its reward
  ...
  do {
    ...
    for (int state = 0; state < num_states; state++)
      {
        double fixed_action_utility = 0.0;
        unsigned int action;
        double meu;
        //if state has available actions then loop
        if (!(p_mdp->numAvailableActions[state] == 0)) {
          fixed_action_utility = calc_eu(p_mdp, state, utilities, policy[state]);
          calc_meu(p_mdp, state, utilities, &meu, &action);
          if (meu > fixed_action_utility){
            policy[state] = action;
            unchanged = false;
          }
        }
